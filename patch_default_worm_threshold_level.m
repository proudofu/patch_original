...
function [DefaultLevel, NumFoundWorms, meanWormSize, Ring] = patch_patch_default_worm_threshold_level(MovieName, background, procFrame, target_numworms, Ring, manual_flag, obj_penalty_coeff)
...
%   Set objects2 equal to the result if we were to manually add no worms when the GUI pops up
    ...
    if(added_worms~=0)
        ...
        [DefaultLevel, NumFoundWorms, meanWormSize] = patch_default_worm_threshold_level(MovieName, background, procFrame, target_numworms, Ring,0, obj_penalty_coeff);
        ...
        while(strcmpi(ok_flag,'OK')==0)
            ...
            [DefaultLevel, NumFoundWorms, meanWormSize] = patch_default_worm_threshold_level(MovieName, background, procFrame, round(coef*target_numworms), Ring,0, obj_penalty_coeff);
            display_found_worms_on_movieframe(Mov, Movsubtract, Ring, NumFoundWorms, obj_penalty_coeff, meanWormSize(1));
            num_worm_text = sprintf('Found %d worms on average',NumFoundWorms);
            ok_flag = questdlg(num_worm_text, sprintf('PID %d %s',Prefs.PID,num_worm_text), 'OK', 'No','OK');
