function splitter(folder, subfolder) % folder is a string in the format YYYYMMDD
% Example call: splitPatchVids('20180705')
    ...
    %mkdir(sprintf('%s_unsplit', folder));
    cd (sprintf('/om/user/kmaher/data/patch_videos/%s', folder))
    %folders = dir; %avi should be in own folder named folder_refeeding_numFields_genotype1_genotype2_genotype3_vid#_Cam#
    %vids = {};
    ...
    %for i= 3:length(folders)
        ...
        % import fields for this camera from the folder folder
        ...
        %movefile(subfolder, sprintf('..//%s_unsplit//', folder));
    end
