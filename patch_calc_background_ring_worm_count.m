function NumFoundWorms = patch_calc_background_ring_worm_count(PathName, moviefile, scaleRing, quick)
...
if(does_this_file_need_making(final_ringfile))
    ...
    if(~does_this_file_need_making(ringfile) || ~isempty(scaleRing))
        if(isempty(Ring.DefaultThresh))
            [DefaultLevel, NumFoundWorms, mws, Ring] = patch_default_worm_threshold_level(sprintf('%s%s%s.avi',PathName, filesep, FilePrefix), background, [], 0, Ring, 1);
            save(ringfile, 'Ring');
        end
    end
