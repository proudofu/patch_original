function splitPatchVids2(date) % date is a string in the format YYYYMMDD
% Example call: splitPatchVids('20180705')
    ...
    c.AdditionaProperties.WallTime = '10:00:00';
    c.saveProfile

    % Open a pool of workers
    p = c.parpool(length(folders) - 2); %#ok Number of workers = number of loops = number of videos in the folder that need splitting
    ...
    %for i= 3:length(folders)
        ...
        % Pre-allocate writers array by assigning last element first
        writers = ObjectArray(length(numFields));
        vidName = makeVidName(nameParts, strains{length(numFields)});
        writers(length(numFields)) = openVidWriter(vidName, v.FrameRate);
