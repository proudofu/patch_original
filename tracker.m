...
% Tracker (as seen above) always has over 3 args
% if(nargin<3)
%     stimulusfile='';
% end

% Tracker always has at least 4 args, and Ringtype is always empty
% if(nargin<4)
%     Ringtype = 'square';
% else
%     if(isempty(Ringtype))
%         Ringtype = 'square'; % this isn't even used throughout the rest of the script
%     end
% end

startFrame = [];
endFrame = [];
% target_numworms = 0; % no longer requires just-in-case initialization, will always be a varargin for patch calls
 
%%% Another dumb nested conditional used to parse varargin
% i=1;
% if(nargin > 4)
    % this statement says: if the first vararg is numeric, do the block. This is always false for patch assay calls.
...
%     if FileName ~= 0
%         saveTracks([localpath, FileName], rawTracks);
%     end
