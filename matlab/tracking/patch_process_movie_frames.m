...
patch_aviread_to_gray; % reset static variables in the aviread_to_gray function

Mov = patch_aviread_to_gray(MovieName, startFrame);
...
for(Frame = startFrame:endFrame)
    ...
    if(p==1 || mod(p,100)==0)
        ...
        patch_aviread_to_gray(MovieName, Frame:min((Frame+100),endFrame));
    end
    ...
    patch_aviread_to_gray(MovieName, Frame, 'clear');
    if(~isempty(ring_mask))
    ...
    if( ~mod(Frame, Prefs.PlotFrameRate) || DEBUG_FLAG==1)
        ...
        %
        %         subplot(1,2,1);
        %         Mov = patch_aviread_to_gray(MovieName, Frame);
        %         imshow(Mov.cdata);
        %         clear('Mov');
        %         hold on;
        %
