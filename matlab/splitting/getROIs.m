function preSplit(date)
    ...
    cd('G:\patch_data_local') % important to match this up to the letter for the hard drive connected that contains the videos
    cd (char(date))
    ...
    for i= 3:length(folders)
        ...
        save (sprintf('%s_%s_ROIs.mat', nameParts{end-1}, nameParts{end}), 'ROIs'); % name each fields struct based on the camera the video was recorded on
        cd ..
    end
end
